...
X, Y = image.imread("res/x1.bmp"), image.imread("res/y3.bmp")
...
def greville_method(M):
    ...
    for i in range(1, n):
        ...
        if np.count_nonzero(dot_product) != 1:
            a_inverted = np.dot(r_a, ai.T) / (1 + np.dot(np.dot(ai, r_a), ai.T))
        else:
            a_inverted = np.dot(z_a, ai.T) / dot_product

        res -= np.dot(a_inverted, np.dot(ai, res))
        res = np.concatenate((res, a_inverted), axis=1)
    return res
